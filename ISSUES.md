...
| **已修复** | RULE-01 | **及时雨规则：补杠即时收益仅在摸到的牌==补杠的牌时生效** |
...
### Phase 3 切换记录（Step 355k）

#### 355k Plateau 分析

V3 对手池运行 ~106k 步（249k→355k），模型从激进型转为平衡型，但在 355k 出现 plateau：

| 指标 | 249k (V3 启动) | 355k (plateau) | 变化 |
|------|---------------|---------------|------|
| 放铳率 | ~45% | ~32% | -13% (显著改善) |
| 和牌点数 | ~6,400 | ~7,100 | +700 (稳步提升) |
| avg_ranking | ~2.59 | ~2.49 | -0.10 |
| point_per_round | ~-340 | ~-20 | 接近零线 |

#### Plateau 根因

1. **对手池陈旧**：最新为 295k，与 355k 差 60k 步，模型已充分适应池中对手
2. **探索率偏高**：epsilon=0.15 在 355k 时产生过多噪声数据
3. **houjuu_penalty=-0.2 过强**：模型过度保守，牺牲进攻机会
4. **学习率偏高**：peak=5e-4 在精调阶段导致参数震荡
5. **Phase 2 超期**：原计划 300k 结束，实际运行到 355k

#### Phase 3 配置变更

| 参数 | Phase 2 值 | Phase 3 值 | 理由 |
|------|-----------|-----------|------|
| boltzmann_epsilon | 0.15 | 0.08 | 减少噪声，提高数据质量 |
| boltzmann_temp | 0.20 | 0.15 | 降低随机性 |
| houjuu_penalty | -0.2 | -0.1 | 减弱惩罚，让 score_diff 主导防守 |
| peak LR | 5e-4 | 3e-4 | 精调阶段降低学习率 |
| final LR | 1e-4 | 5e-5 | 降低最终 LR |
| warm_up_steps | 2000 | 3000 | 平滑过渡 |

#### 对手池状态

| 检查点 | 风格 | 备注 |
|--------|------|------|
| 130k | 保守型 | Phase 2 早期，保留用于多样性 |
| 200k | 中间型 | 纯 score_diff 初期 |
| 249k | 激进型 | V3 实施点 |
| 295k | 平衡型 | V3 运行中 |
| 355k | 当前最新 | Phase 3 起点，newest_weight=3.0 |

### Phase 4 记录（Step 485k — 580k）

#### Phase 3b→4 切换 (485k)

关闭所有奖励塑形（action_bonus_enabled=false, houjuu_penalty=0），进入纯 score_diff 驱动。

#### Phase 4 问题

1. **对手池过乱**（550k 发现）：7 个检查点（130k-485k），67% 对局面对远弱于自己的对手
2. **低番速胡退化**：agari_point 从 6666(510k) 降至 5964(555k)，模型学会打弱对手的投机策略
3. **对手池清理**（550k）：删除 130k/200k/249k/295k/355k，仅保留 420k/485k/550k

清理后短期有改善（agari_point 回升至 6548），但整体 point_per_round 仍为负值，无明显突破。

### Phase 5 梯度净化记录（Step 580k）

#### 580k 梯度审计

对奖励系统、损失函数、辅助任务进行深度审计，发现 Brain 编码器梯度分布严重失衡：

| 损失项 | 原始值 | 权重 | 加权值 | 占比 |
|--------|--------|------|--------|------|
| dqn_loss | 0.229 | 1.0 | 0.229 | 43.2% |
| next_rank_loss | 1.156 | 0.25 | 0.289 | **54.5%** |
| ding_que_ce_loss | 0.003 | 2.0 | 0.006 | 1.1% |
| opp_wait_loss | 0.046 | 0.1 | 0.005 | 0.9% |
| ding_que_dqn_ce_loss | 0.004 | 0.1 | 0.0004 | 0.08% |

**核心问题**：next_rank 辅助任务占总梯度 54.5%，超过 DQN 主任务。编码器学习方向被"预测排名"主导而非"选择好动作"。且血战到底单局结算，next_rank 预测价值有限。

同时发现 ding_que_bonus (±0.02) 仍在 Q-target 上活跃，Phase 4 并非真正的"纯 score_diff"。

#### Phase 5 配置变更

| 参数 | Phase 4 值 | Phase 5 值 | 理由 |
|------|-----------|-----------|------|
| next_rank_weight | 0.25 | **0.0** | 血战到底无需排名预测；占 54.5% 梯度 |
| ding_que_aux_enabled | true | **false** | 去除 Q-target 上隐藏的 ±0.02 奖励塑形 |
| ding_que_ce_weight | 2.0 | **1.0** | 定缺已收敛 (CE=0.003)，降权 |
| ding_que_dqn_ce_weight | 0.1 | **0.0** | 消除 DQN 竞争梯度 |

变更后 DQN 梯度占比从 43% 升至 **95.4%**。
...
| 420k | 20% | 强对手 |
| 485k | 20% | 接近当前水平 |
| 550k | 60% (newest_weight=3.0) | 自我对弈 |

---

### 超人类可行性分析 (680k 步, 2026-02-15)

#### 分析背景

Phase 5 梯度净化运行 100k 步 (580k→680k)，DQN 梯度占比从 43% 升至 95.4%。
训练指标有缓慢改善但未达预期目标。需评估当前架构的理论天花板。

#### 680k 步关键指标

| 指标 | 值 | 超人类目标 | 差距 |
|------|-----|-----------|------|
| houjuu_rate | 32.5% | < 18% | 需减半 |
| agari_point | 5915 | > 7500 | 需 +27% |
| point_per_round | -344 | > +500 | 需翻转 |
| avg_ranking | 2.520 | < 2.05 | |
| dqn_match_rate | 99.94% | >99.9% | 已达标 |

#### 识别的 4 个结构性瓶颈

1. **无推理时搜索**: 单次前向传播天花板。所有超人类系统 (AlphaZero/Suphx/Pluribus) 都有搜索。
2. **MC 回报信用分配差**: TD(λ=1.0) 无 V(s') bootstrap，一局 20 步的 reward 均摊到每步，防守信号被稀释。
3. **DQN 无博弈论基础**: 4 人游戏当单智能体 MDP，可能有策略循环和可利用性。
4. **训练规模**: ~4.35M 局，远低于 AlphaZero/OpenAI Five 级别。

#### 改进路线图

| 优先级 | 技术 | Phase | 难度 | 预期效果 |
|--------|------|-------|------|---------|
| 1 | Target Network + TD(λ=0.95) | Phase 6 | 中 | 放铳率 32%→25%，信用分配改善 |
| 2 | Oracle Guiding | Phase 7A | 高 | 跳过基础学习，直接优化高级策略 |
| 3 | 推理时搜索 | Phase 7B | 高 | 突破单次前向传播天花板 |
| 4 | Population-Based Training | Phase 7C | 极高 | 消除策略循环 |

#### 结论

当前架构天花板: 「中等业余」。
+Target Network: 「强业余」。
+Oracle Guiding 或搜索: 「超人类」可期。

详细分析见 TRAINING_PLAN.md §12。
