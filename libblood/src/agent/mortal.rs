...
impl MortalBatchAgent {
    ...
    fn evaluate(&mut self) -> Result<()> {
        ...
        (self.actions, self.q_values, self.masks_recv, self.is_greedy) = Python::with_gil(|py| {
            // Use list of individual arrays - let Python/NumPy handle stacking
            // This is faster than Rust-side ndarray::stack
            let states: Vec<_> = sync_fields
                .states
                .drain(..)
                .map(|v| PyArray2::from_owned_array(py, v).unbind())
                .collect();
            let masks: Vec<_> = sync_fields
                .masks
                .drain(..)
                .map(|v| PyArray1::from_owned_array(py, v).unbind())
                .collect();
