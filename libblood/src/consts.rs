...
pub const fn obs_shape(version: u32) -> (usize, usize) {
    ...
    match version {
        ...
        4 => (1066, 27), // 基于实际运行结果设置：idx=1066（最大）
                         // 理论计算：SP table 编码之前 875 行 + SP table 编码（最大路径）111 行 = 986 行
                         // 实际需要 1066 行，差异 80 行
                         // 已修复：single_player_tables() 失败路径在 can_discard=True 时需要额外的 2 行
                         // 问题：idx 持续增加（1024 → 1026 → 1038 → 1050 → 1052 → 1054 → 1062/1066）
                         // 说明不同的编码路径需要不同的行数，可能存在编码逻辑不一致或遗漏
                         // 可能原因：
                         //   1. 某些编码步骤被遗漏（例如某些条件分支）
                         //   2. 某些编码步骤的计算不准确
                         //   3. 某些编码路径需要额外的空间
                         // TODO: 需要深入分析编码逻辑，找出差异 80 行的来源，并优化以减少空间使用
        _ => unreachable!(),
    }
}
