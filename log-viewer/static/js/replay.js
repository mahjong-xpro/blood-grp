    ...
    setup() {
        ...
        const processEvent = (event) => {
            ...
            switch (type) {
                ...
                case 'pon':
                case 'chi': // Blood battle no chi, but generic support
                    if (p && event.consumed) {
                        // Remove consumed
                        event.consumed.forEach(t => removeTile(p, t));
                        // Add to fuuro
                        // [pai, ...consumed]
                        p.fuuro.push([event.pai, ...event.consumed]);
                        p.lastAction = type;
                        currentPlayer.value = actor;
                        // Determine who discarded it?? 
                        // In MJAI 'target' is who discarded.
                        // We should ideally mark that tile as "claimed" in target's kawa?
                        // Visuals don't strictly require it for replay but good for polish.
                    }
                    break;
                ...
                case 'ankan':
                    if (p && event.consumed) {
                        event.consumed.forEach(t => removeTile(p, t));
                        p.fuuro.push([...event.consumed]); // 4 tiles
                        p.lastAction = 'ankan';
                    }
                    break;
                ...
                case 'kakan':
                    if (p && event.pai) {
                        removeTile(p, event.pai);
                        // Find pon
                        const pon = p.fuuro.find(m => m.length === 3 && m.includes(event.pai));
                        if (pon) pon.push(event.pai);
                        p.lastAction = 'kakan';
                    }
                    break;
        ...
        return {
            ...
            players, currentPlayer, lastDiscard,
