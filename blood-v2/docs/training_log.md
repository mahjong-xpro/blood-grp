...
### Phase 2b-续: Competitive Distill — LR 优化后（进行中 🔄 ~573K/4M 新计数）

> 运行名: `blood_v2_competitive_distill/.summary/0`（step counter 已重置）
> 配置: `configs/competitive_distill.yaml` | 实际总训练量: ~2.2M(旧) + 573K(新) ≈ 2.8M steps
> 关键变更: `lr_adaptive_min: 5e-5`, `lr_schedule_kl_threshold: 0.0005`, `arena_eval_games: 100`
> Elo tracker 状态已继承（起始 elo_games=550）

#### 优化效果对比

| 指标 | 优化前 (旧 2.2M 均值) | 优化后 (@573K) | 变化 | 评估 |
|------|----------------------|----------------|------|------|
| `train/actual_lr` | 2e-5 (76.6% 锁死) | **5e-5** (稳定) | ↑ 2.5x | ✅ 核心修复 |
| `train/kl_divergence` | 0.0004 | **0.0016** | ↑ 4x | ✅ 策略更新更积极 |
| `train/fraction_clipped` | 1.1% | **11.6%** | ↑ 10x | ⚠️ 偏高，需观察 |
| `train/entropy` | 0.92 | **1.07** | ↑ 0.15 | ✅ entropy schedule 重启 |
| `train/grad_norm` | 1.50 (均值) | **2.0** (频繁贴满) | ↑ | ⚠️ 梯度压力增大 |
| `blood/elo_current` | 1441 (末值) | **1484** | ↑ +43 | ✅ 回升，但有波动 |

#### 奖励 / 目标

| 指标 | @8K | @100K | @200K | @300K | @400K | @500K | @573K | 评估 |
|------|-----|-------|-------|-------|-------|-------|-------|------|
| `reward/reward` | +0.62 | -0.25 | -0.21 | -0.19 | -0.18 | -0.04 | **-0.20** | ✅ 零和收敛 |
...
| 指标 | @8K | @100K | @200K | @300K | @400K | @500K | @573K | 评估 |
|------|-----|-------|-------|-------|-------|-------|-------|------|
| `train/loss` | 0.42 | 0.73 | 0.83 | 1.26 | 1.02 | 0.92 | **0.99** | ⚠️ 峰值后回落中 |
| `train/value_loss` | 0.46 | 0.79 | 0.88 | 1.31 | 1.08 | 0.97 | **1.04** | ⚠️ 同上，趋势向好 |
...
| 13 | 418K→426K | 1567→1484 | 697→750 | 0.54 | 2.36 | 101650 | ⚠️ Elo 先升后降 |

> 注: Eval 13 分两步记录（418K: +47 局, 426K: +53 局），合计 100 局。Elo 在中间点达到 1567（历史新高），完成后回落到 1484。
...
|------|-----|------|
| `len/len` | 18.5 步 | ✅ 正常 |
| `blood/league_pool_size` | 50（未变化） | — |
| `blood/sched_exploration_loss_coeff` | 0.049 → **0.049** | ✅ cosine 衰减中（重启后） |

#### 分析（2026-02-26 00:56 CST）

**LR 优化效果: ✅ 正面，但需要继续观察**

1. **Elo 曾达到 1567（历史新高）**: Eval 13 中间点 Elo 1567，超过旧 run 峰值 1541。虽然完成后回落到 1484，但说明模型有能力达到更高水平。
2. **Arena 指标改善**: win_rate 0.54, avg_rank 2.36（优于旧 run 均值 2.50+），avg_score 101650（高于初始 100000）。
3. **value_loss 开始回落**: 从峰值 1.31(@300K) 降到 1.04(@573K)，趋势向好。
4. **KL 稳定在 0.0016**: 比旧 run 的 0.0004 高 4x，策略在积极更新。

**需要关注**:

1. **fraction_clipped 11.6%**: 持续上升（9.4%→11.6%）。`ppo_clip_ratio=0.1` 下超过 10% 说明策略更新幅度较大。如果超过 15%，建议将 `ppo_clip_ratio` 从 0.1 提升到 0.15。
2. **Elo 波动大**: 1498→1517→1567→1484，振幅 ~80。这可能是 100 局评估的正常方差，也可能是策略不稳定。需要更多评估点确认趋势。
3. **grad_norm 全程贴满 2.0**: 更高的 LR 导致更大的梯度。如果 value_loss 稳定后仍贴满，考虑 `max_grad_norm: 2.0 → 3.0`。
