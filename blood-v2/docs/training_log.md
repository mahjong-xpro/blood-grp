...
### Phase 2b-续: Competitive Distill — 实时数据更新（🔄 ~2.33M/4M 新计数）

> 运行名: `blood_v2_competitive_distill/.summary/0`（step counter 已重置）
> 配置: `configs/competitive_distill.yaml` | 实际总训练量: ~2.2M(旧) + 2.33M(新) ≈ 4.5M steps
> 关键变更: 优化#1 `lr_adaptive_min: 5e-5` + 优化#2 `max_grad_norm: 3.0, ppo_clip_ratio: 0.15`
> Elo tracker 状态已继承（起始 elo_games=550，当前 1653）
> 数据来源: TensorBoard API `http://localhost:6006` @ 2026-02-26 03:30 CST

#### 两次优化综合效果对比

| 指标 | 旧 run (2.2M) | 优化#1 only (0~1.06M) | 优化#1+#2 (1.07M~2.33M) | 评估 |
|------|--------------|----------------------|------------------------|------|
| `train/actual_lr` | 2e-5 (76.6% 锁死) | 5e-5 (100% 锁死) | **5e-5** (100% 锁死) | ⚠️ 仍锁死 |
| `train/kl_divergence` | 0.0004 | 0.0015 | **0.0018** (最新 0.0038) | ✅ 策略更新活跃 |
| `train/fraction_clipped` | 1.1% | 9.5% | **4.1%** | ✅ 达标 |
| `train/grad_norm` | 1.50 (29% clip@2.0) | 1.85 (69% clip@2.0) | **2.50** (41% clip@3.0) | ⚠️ 稳定 |
| `train/value_loss` | 0.89 | 0.93 | **0.81** (最新 0.59) | ✅ 下降 |
| `train/entropy` | 0.92 | 1.01 | **0.87** | ✅ cosine 衰减中 |
| `blood/elo_current` | 1441 | 1500 | **1525** (峰值 1549) | ✅ 稳定 |

#### 奖励 / 目标

| 指标 | @8K | @200K | @688K | @1M | @1.72M | @2.33M | 评估 |
|------|-----|-------|-------|------|--------|--------|------|
| `reward/reward` | +0.62 | -0.21 | -0.15 | -0.10 | +0.06 | **-0.02** | ✅ 零和 |

#### 损失

| 指标 | @8K | @200K | @688K | @1M | @1.72M | @2.33M | 评估 |
|------|-----|-------|-------|------|--------|--------|------|
| `train/value_loss` | 0.46 | 0.88 | 1.32 | 1.01 | 0.54 | **0.59** | ✅ 下降 |
...
| 18 | 1933K | 1474 | — | — | — | — | ⚠️ |
| 19 | 2130K | 1520 | — | — | — | — | ✅ |
| 20 | 2138K | 1549 | 1650 | **0.60** | **2.32** | **101160** | 🎉 |
| 21 | 2335K | **1525** | 1653 | **0.60** | **2.32** | **101160** | ✅ 稳定 |
...
```
Elo 轨迹（关键节点）:
1507→...→1446(低谷1)→...→1554(峰值1)→...→1467(低谷2)→...→1549(峰值2)→1525(当前)
```
...
- 峰值 2: 1549 (@2.14M) — 回升 +82
- 当前: 1525 (@2.33M) — 微降 -24

**关键洞察**: 低谷在抬升（1446→1467 +21），峰值基本维持（1554→1549 -5）。Elo 稳定在 1520+ 区间。
...
Phase 1: Warmup (2M) → Phase 1.5: Transition (500K) → Phase 2a: Competitive (1M)
    ...
    → Phase 2b-续 (2.33M) → 当前位置 ⬅️
    
总训练量: ~8.0M env steps | Phase 2b 实际: ~4.5M steps（已超过原定 4M 目标）
Elo: 1525 @2.33M（从峰值 1549 微降，稳定在 1520+ 区间）
```

#### 当前状态: ✅ 训练健康，Elo 稳定在 1520+，Phase 3 进入时机成熟
...
### Phase 3: Elite（进行中 🔄 ~5.64M/50M steps）

> 运行名: `blood_v2_elite/.summary/0`
> 配置: `configs/elite.yaml` | 目标: 50M steps | 当前: ~5.64M steps (11.3%)
> 前置 checkpoint: Phase 2b competitive_distill 最佳 checkpoint
> 核心变化: `gamma: 0.999`, `gae_lambda: 0.97`, `rtpa_enabled: true`, `ismce_enabled: true`
> 数据来源: TensorBoard API `http://localhost:6006` @ 2026-02-26 10:55 CST
...
#### 训练指标（@5.64M steps — 11.3% 完成）

| 指标 | @8K | @532K | @1.33M | @5.19M | @5.64M (当前) | 评估 |
|------|-----|-------|--------|--------|--------------|------|
| `train/actual_lr` | 1.33e-4 | 5.9e-5 | 2.0e-4 | 1.12e-4 | **5.0e-5** | ⚠️ 锁死 |
| `train/kl_divergence` | 0.0096 | 0.0017 | 0.0031 | 0.0016 | **0.0052** | ⚠️ 跳升 |
| `train/fraction_clipped` | 11.3% | 2.1% | 5.8% | 0.7% | **4.8%** | ✅ 正常 |
| `train/grad_norm` | 2.36 | 2.32 | 1.74 | 1.74 | **0.87** | ✅ 低 |
| `train/entropy` | 0.57 | 0.76 | 0.60 | 0.42 | **0.40** | ⚠️ 警戒线 |
| `train/value_loss` | 1.33 | 1.00 | 0.61 | 0.68 | **0.95** | ⚠️ 上升 |
| `reward/reward` | +2.43 | -0.11 | +0.01 | -0.03 | **-0.06** | ✅ 零和 |
| `blood/elo_current` | 1500 | 1483 | 1537 | 1526 | **1474** | ❌ 下降 |

#### LR 动态调整统计（全程 5.19M steps）

| 指标 | 值 | 评估 |
|------|-----|------|
| LR 均值 | **1.06e-4** | ✅ Phase 2b 的 **2.1x** |
| LR 范围 | 5e-5 ~ 2e-4 | ✅ 全范围使用 |
| 锁死在 5e-5 的比例 | **33.0%** | ⚠️ 从初期 10% 上升 |
| grad_norm clip@3.0 | **13.7%** | ✅ 非常健康 |

#### Arena 评估 📊（每 500K 步，100 局 vs RuleBot，Elo 累积 1000 局）

| # | Step | Elo (中间→最终) | Elo Games | Win Rate | Avg Rank | Avg Score | 评估 |
|---|------|----------------|-----------|----------|----------|-----------|------|
| 1 | 516K→524K | 1469→1483 | 100 | 0.49 | 2.63 | 98880 | ⚠️ 适应期 |
| 2 | 1024K→1032K | 1450→1537 | 200 | 0.55 | 2.48 | 99610 | ✅ |
| 3 | 1532K→1540K | 1489→1473 | 300 | — | — | — | ⚠️ |
| 4 | 2040K→2048K | 1469→1484 | 400 | — | — | — | ⚠️ |
| 5 | 2548K→2556K | 1545→1489 | 500 | — | — | — | ⚠️ |
| 6 | 3056K→3064K | 1435→1501 | 600 | — | — | — | ⚠️ 低谷 |
| 7 | 3564K→3572K | 1513→1519 | 700 | — | — | — | ✅ |
| 8 | 4071K→4080K | 1509→1510 | 800 | — | — | — | ✅ |
| 9 | 4579K | 1548 | 900 | — | — | — | ✅ 峰值 |
| 10 | 5087K→5095K | 1545→1526 | 1000 | 0.54 | 2.33 | 100970 | ✅ |
| 11 | 5595K→5603K | 1475→**1474** | 1100 | **0.45** | **2.62** | **99560** | ❌ 下降 |

#### 🔍 深度分析（2026-02-26 10:55 CST）

##### 1. 训练健康度总评: ⚠️ Elo 下降 + entropy 触及警戒线

Elo 从峰值 1548(@4.58M) 连续下降到 1474(@5.60M)，entropy 降到 0.40 触及警戒线。需要密切关注但暂不干预——Phase 2b 也有类似的 ~100 点回落后回升的模式。

##### 2. Elo 轨迹分析

```
Elo 轨迹:
1500→...→1435(低谷1)→...→1548(峰值)→1545→1526→1475→1474(当前)
                                                          ↑ 连续下降
```

**模式**: "锯齿震荡"继续。Elo 在 1435~1548 之间波动。
- 低谷 1: 1435 (@3.06M)
- 峰值: 1548 (@4.58M)
- 当前: 1474 (@5.60M) — 从峰值回落 -74

**对比 Phase 2b**: Phase 2b 也有 1554→1467 的 -87 回落，之后回升到 1549。当前回落幅度 -74 在正常范围内。

##### 3. ⚠️ entropy 触及警戒线 (0.40)

entropy 衰减轨迹: 0.76→0.60→0.42→**0.40**

entropy 0.40 意味着策略非常确定。elite 的 entropy schedule `cosine,0.02,0.005,0,40000000` 在 5.6M 步时 coeff ≈ 0.020（几乎没衰减，因为 cosine 前期变化慢）。entropy 下降主要是策略自身收敛导致的，而非 schedule 压低。

**风险**: 如果 entropy 继续下降到 < 0.3，策略可能过度确定，失去适应能力。

##### 4. ⚠️ LR 和 KL 波动

最新 LR 锁死在 5e-5，KL 跳到 0.0052（高于均值 0.002）。这说明策略在某些 minibatch 中更新幅度较大，触发了 KL adaptive 的降 LR 机制。这是正常的波动，不是持续问题。

##### 5. 下一步建议

**暂不干预**。再观察 2 次 arena eval（~1M steps）。

| 情景 | 触发条件 | 行动 |
|------|---------|------|
| Elo 回升到 > 1520 | 正常波动 | 继续运行 |
| Elo 继续下降 < 1430 | 策略退化 | 提升 entropy（见下方） |
| entropy < 0.30 | 策略僵化 | 立即提升 entropy |

**如果需要提升 entropy**:
```yaml
# 方案 A: 提升 entropy schedule 起始值
blood_schedule_entropy: "cosine,0.03,0.005,0,40000000"  # 从 0.02 提升到 0.03

# 方案 B: 直接提升 exploration_loss_coeff
exploration_loss_coeff: 0.03  # 从 0.01 提升到 0.03
```
...
| 指标 | 当前值 | 健康范围 | 警报阈值 |
|------|--------|---------|----------|
| `blood/elo_current` | **1474** ⚠️ | > 1500 | < 1430 连续 2 次 |
| `train/entropy` | **0.40** ⚠️ | > 0.3 | < 0.25 |
| `train/actual_lr` | 5e-5 (锁死) | 波动中 | 持续锁死 > 500K 步 |
| `train/value_loss` | 0.95 | < 1.0 | > 1.5 持续 |
| `train/grad_norm` clip@3.0 | 13.7% | < 30% | > 50% |
