...
class TestSuitAwareResNetEncoder:
    ...
    def test_output_shape(self):
        ...
        out = enc({"obs": obs})
        assert out.shape == (4, 64 * NUM_TILES)
...
class TestAuxHead:
    def test_forward_shape(self):
        ...
        dq_logits, ow_logits = head(features)
        assert dq_logits.shape == (8, 3, 3)
        assert ow_logits.shape == (8, 81)

    def test_loss_computes(self):
        head = AuxHead(in_dim=256, hidden=128)
        features = torch.randn(8, 256)
        dq_labels = torch.randint(0, 3, (8, 3))
        ow_labels = torch.zeros(8, 81)
        loss = head.loss(features, dq_labels, ow_labels)
        assert loss.ndim == 0
        assert loss.item() > 0

    def test_ignore_index(self):
        """DQ label = 3 should be ignored via ignore_index.

        When all labels are unknown (3), DQ CE returns 0 but BCE on ow
        still contributes, so the total loss is finite only from ow_loss.
        When some labels are valid, they contribute normally.
        """
        head = AuxHead(in_dim=256, hidden=128)
        features = torch.randn(4, 256)

        dq_labels = torch.randint(0, 3, (4, 3))
        dq_labels[0, :] = 3  # first sample all unknown
...
class TestOracleEncoder:
    def test_output_shape(self):
        oracle = OracleEncoder(obs_channels=430, conv_ch=32, num_blocks=2, out_dim=128, action_dim=34)
        obs = torch.randn(4, 430 * NUM_TILES)
        logits = oracle(obs)
...
class TestPolicyModel:
    def test_forward(self):
        model = PolicyModel(obs_channels=DEFAULT_OBS_CHANNELS, conv_ch=32, num_blocks=2, encoder_out=128)
...
class TestRTPA:
    def test_temperature_range(self):
        ...
        temp = rtpa.compute_temperature(
            ...
            my_score=60000, avg_opponent_score=60000.0, wall_remaining=50,
        )
        assert 0.3 <= temp <= 3.0
